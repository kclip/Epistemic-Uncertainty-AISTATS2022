...
if __name__ == '__main__':
    ...
    results_path = results_dir +  '/hyper_mi_exp_mode_' + str(args.exp_mode) + '.mat'
    ...
    results_dict = {}
    hyper_mi_eval = torch.zeros(args.num_indep_exp_total, len(num_samples_iter), len(num_tasks_iter))
    ...
    for ind_exp in range(args.num_indep_exp_total):
        ...
        for ind_num_samples_iterative in range(len(num_samples_iter)):
            for ind_num_meta_tr_tasks_iterative in range(len(num_tasks_iter)): 
                ...
                dataset_marg['u'] = dataset_marg_tmp['u'] # this makes 'u' follows marginal distribution not joint distribution
                variable_list_mi_1_1 = ['u', 'z_mtr',]
                ...
                print('------------------------------------------------------------------')
                print('num_samples: ', num_samples, 'num_meta_tr_tasks: ', num_meta_tr_tasks, 'hyperpara mi: ', mi_1_1/(num_samples*num_meta_tr_tasks))
                hyper_mi_eval[ind_exp, ind_num_samples_iterative, ind_num_meta_tr_tasks_iterative] = mi_1_1/(num_samples*num_meta_tr_tasks)
                results_dict = {}
                results_dict['mi_hyperpara_div_Nm'] = hyper_mi_eval.data.numpy()
