...
import copy
import os
from utils.total_dataset_gen_forward import Total_dataset_gen_forward, trim_to_mN, selecting_essentials_for_mi_para_via_avg
import time
import scipy.io as sio

N_MAX = 100 # number of total meta-training tasks (this is determined from datagen.py) -- we can still use the generated dataset but with increased sample size from this way (one task has N_MAX number of samples for mi_para)

def parse_args():
    parser = argparse.ArgumentParser(description='code for information-theoretic bounds')
    parser.add_argument('--num_u_samples', type=int, default=3000, help='number of different true U (hyperparameter) to average over U')
...
if __name__ == '__main__':
    ...
    results_path = results_dir +  '/para_mi_exp_mode_' + str(args.exp_mode) + '.mat'
    if args.num_samples is None:
        num_samples_iter = [1, 2, 5, 50]
    else:
        num_samples_iter = [args.num_samples]
    if args.num_meta_tr_tasks is None:
        num_tasks_iter = [1] # this means nothing essentially 
    else:
        num_tasks_iter = [args.num_meta_tr_tasks]
    results_dict = {}
    mi_para_eval = torch.zeros(args.num_indep_exp_total, len(num_samples_iter), len(num_tasks_iter))
    ...
    for ind_exp in range(args.num_indep_exp_total):
        ...
        for ind_num_samples_iterative in range(len(num_samples_iter)):
            for ind_num_meta_tr_tasks_iterative in range(len(num_tasks_iter)): 
                ...
                mi_over_u = torch.zeros(num_u_samples)
                if os.path.exists(path_for_dataset_joint) and os.path.exists(path_for_dataset_marg): # 0 for joint, 1 for marg
                    ...
                    dataset_joint_original = trim_to_mN(dataset_joint_original_max, num_u_samples, N_MAX, num_samples)
                    dataset_marg_tmp_original = trim_to_mN(dataset_marg_tmp_original_max, num_u_samples, N_MAX, num_samples)
                else:
                    print('run datagen.py (total_data_gen.sh) first!')
                    raise NotImplementedError
                for ind_u in range(num_u_samples):
                    dataset_joint = dataset_joint_original
                    dataset_marg_tmp = dataset_marg_tmp_original
                    dataset_marg = copy.deepcopy(dataset_joint)
                    dataset_marg['w_mtr'] = dataset_marg_tmp['w_mtr'] # this makes 'w_mtr' follows marginal distribution not joint distribution
                    variable_list_mi_1_1 = ['w_mtr', 'z_mtr']
                    len_data = dataset_marg['w_mte'].shape[1] + dataset_marg['z_mte'].shape[1]
                    flatten_dataset_mi_1_1_joint = selecting_essentials_for_mi_para_via_avg(ind_u, dataset_joint, variable_list_mi_1_1, N_MAX, len_data, num_samples)
                    flatten_dataset_mi_1_1_marg = selecting_essentials_for_mi_para_via_avg(ind_u, dataset_marg, variable_list_mi_1_1, N_MAX, len_data, num_samples)
                    ...
                    mi_over_u[ind_u] = mi_1_1
                print('------------------------------------------------------------------')
                print('num_samples: ', num_samples, 'num_meta_tr_tasks: ', num_meta_tr_tasks, 'para MI: ', torch.mean(mi_over_u)/(num_samples))
                mi_para_eval[ind_exp, ind_num_samples_iterative, ind_num_meta_tr_tasks_iterative] = torch.mean(mi_over_u)/(num_samples)
                results_dict = {}
                results_dict['mi_para_div_m'] = mi_para_eval.data.numpy()
